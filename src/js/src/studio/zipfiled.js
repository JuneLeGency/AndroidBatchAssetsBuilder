...
studio.forms.ZipField = studio.forms.Field.extend({
  ...
  renderValueAndNotifyChanged_: function() {
    ...
    switch (this.valueType_) {
      ...
      case 'clipart':
        params=this.imageParams_;
        console.log("处理图片一共:"+params.length+"个");
        eximage();
        break;
    ...
    function eximage(){
      ...
      if(current==params.length){
        return me.form_.notifyChanged_(me);
      }
      image_param= params[current];
      if (image_param.canvgSvgText || image_param.canvgSvgUri) {
        ...
        canvg(
          canvas,
          image_param.canvgSvgText ||
            image_param.canvgSvgUri,
          {
            scaleWidth: size.w,
            scaleHeight: size.h,
            ignoreMouse: true,
      ...
      } else if (image_param.uri) {
        imagelib.loadFromUri(image_param.uri, function(img) {
          var size = {
            w: img.naturalWidth,
            h: img.naturalHeight
          };
          if (image_param.isSvg && image_param.maxFinalSize) {
            size = {
...
studio.forms.ZipField.readFile = function(files,i,datas,callback){
  ...
  if(i == files.length)
    ...
    file = files[i];
    if (!file) {
      alert('请选择有效的文件格式(PNG, JPG, GIF, SVG, etc.)');
      return;
    }
